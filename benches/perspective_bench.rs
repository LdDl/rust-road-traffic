...
use rust_road_traffic::lib::spatial::{
    PerspectiveTransform, compute_perspective_matrix, lonlat_to_meters,
};
...
fn setup_test_data() -> ([(f32, f32); 4], [(f32, f32); 4]) {
    ...
    let dst_meters: [(f32, f32); 4] =
        core::array::from_fn(|i| lonlat_to_meters(dst_wgs84[i].0, dst_wgs84[i].1));
    (src, dst_meters)
}
...
fn bench_transform_point(c: &mut Criterion) {
    ...
    c.bench_function("transform_point", |b| {
        b.iter(|| black_box(transform.transform(black_box(800.0), black_box(500.0))))
    });
}
